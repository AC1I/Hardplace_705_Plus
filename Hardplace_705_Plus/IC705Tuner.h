...
public:
  CIC_705Tuner(
    ...
    : m_uTunePwr((255 * 25) / 100),     // 2.5 Watts (need to keep it between 1 and 5 watts)
      m_uTunePwrMax((255 * 45) / 100),  // Don't tune with greater than 4.5 Watts
      m_uTunePwrMin((255 * 10) / 100),  // Can't tune with less than 1 Watt
      m_RTTY(4),                        // RTTY, CW is 3
      m_uFilterWidthNormal(2),
      m_fTuning(false),
      m_r705(r705),
...
private:
  const unsigned       m_uTunePwr;
  const unsigned       m_uTunePwrMax;
  const unsigned       m_uTunePwrMin;
  const unsigned       m_RTTY;
  const unsigned       m_uFilterWidthNormal;
  bool                 m_fTuning;
