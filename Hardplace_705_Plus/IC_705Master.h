...
public:
  CIC_705MasterDevice(
    IBluetooth& rBluetooth, unsigned char uchRigAddress,
    Stream& rDevice, uint32_t uBaudrate = 115200, unsigned long ulTimeout = 1000)
    : CSerialDevice(rDevice, uBaudrate, ulTimeout),
      CICOMReq(uchRigAddress),
      CBoundDevice(static_cast<CBoundDevice::eDeviceClass>(CTeensy::eBoundDeviceTypes::IC_705Master)),
      m_rBluetooth(rBluetooth) {
  }
  ~CIC_705MasterDevice() {
...
public:
  bool isConnected(void) {
    return m_rBluetooth.BluetoothConnected();
  }
  bool connectionStateChanged(void) {
    return m_rBluetooth.BluetoothConnectionStateChanged();
  }
  void Pair(void) {
    m_rBluetooth.BluetoothPair();
  }
  void clearPairing(void) {
    Pair();
  }
  void DisconnectBoundDevice(void) {
    m_rBluetooth.BluetoothDisconnect();
  }
