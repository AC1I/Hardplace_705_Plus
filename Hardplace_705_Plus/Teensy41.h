...
Interface IBluetooth {
  virtual bool BluetoothConnected(void) = 0;
  virtual bool BluetoothConnectionStateChanged(void) = 0;

  virtual void BluetoothDisconnect(void) = 0;
  virtual void BluetoothPair(void) = 0;

  virtual bool BluetoothFlowControl(uint8_t & ruRTS, uint8_t & ruCTS) = 0;
};
...
public:
  ...
  void setup(void) {
    ...
    m_bState.attach(BT_ICOM_STATE, INPUT);
    m_bState.interval(m_uDebounceInterval);
...
public:
  ...
  virtual bool BluetoothConnected(void) {
    bool fConnected(m_bState.read() != LOW);

    if (fConnected) {
      digitalWrite(BT_ICOM_PAIR, LOW);
      digitalWrite(BT_ICOM_DISCONNECT, LOW);
    }
    return fConnected;
  }
  virtual bool BluetoothConnectionStateChanged(void) {
    return m_bState.changed();
  }
  virtual void BluetoothDisconnect(void) {
    digitalWrite(BT_ICOM_DISCONNECT, HIGH);
  }
  virtual void BluetoothPair(void) {
    digitalWrite(BT_ICOM_PAIR, HIGH);
  }
  virtual bool BluetoothFlowControl(uint8_t& ruRTS, uint8_t& ruCTS) {
    ruRTS = BT_ICOM_RTS;
    ruCTS = BT_ICOM_CTS;
    return true;
  }
...
#define percentToHex(_value_) ((_value_ * 255) / 100)
      uint8_t m_uchMaxPwrAnt[2][11] = {
        { percentToHex(50), percentToHex(94), percentToHex(74), percentToHex(83),  // 6 10 12 15
          percentToHex(88), percentToHex(76), percentToHex(35), percentToHex(70),  // 17 20 30 40
          percentToHex(14), percentToHex(49), percentToHex(00) },                  // 60 80 160
...
public:
  ...
  bool SendEnabled(eHardrock eWhichHardrock) const {
    ...
    if (nIndex >= 0) {
      return eWhichHardrock == eHardrock::QRP
             || (eWhichHardrock == eHardrock::A && digitalRead(m_aPTTEnableMap[nIndex]) == LOW)
             || (eWhichHardrock == eHardrock::B && !digitalRead(m_aPTTEnableMap[nIndex]) == LOW);
    }
    return false;
  }
...
protected:
  ...
  uint8_t        m_aInitialPwr[3][11] = {
           { percentToHex(10), percentToHex(8), percentToHex(6), percentToHex(8),     // 6 10 12 15
             percentToHex(8), percentToHex(6), percentToHex(7), percentToHex(7),      // 17 20 30 40
             percentToHex(6), percentToHex(6), percentToHex(00) },                    // 60 80 160
           { percentToHex(10), percentToHex(8), percentToHex(6), percentToHex(8),     // 6 10 12 15
             percentToHex(8), percentToHex(6), percentToHex(7), percentToHex(7),      // 17 20 30 40
             percentToHex(6), percentToHex(6), percentToHex(00) },                    // 60 80 160
           { percentToHex(50), percentToHex(50), percentToHex(50), percentToHex(50),  // 6 10 12 15
             percentToHex(50), percentToHex(50), percentToHex(50), percentToHex(50),  // 17 20 30 40
             percentToHex(50), percentToHex(50), 0 }                                  // 60 80 160
  };
#undef percentToHex
  ...
  const int         m_aPTTEnableMap[11] = { HR_6M_PTT_Enable, HR_10M_PTT_Enable, HR_12M_PTT_Enable,
                                            HR_15M_PTT_Enable, HR_17M_PTT_Enable, HR_20M_PTT_Enable,
                                            HR_30M_PTT_Enable, HR_40M_PTT_Enable, HR_60M_PTT_Enable,
                                            HR_80M_PTT_Enable, HR_160M_PTT_Enable };
  Watchdog          m_Watchdog;
